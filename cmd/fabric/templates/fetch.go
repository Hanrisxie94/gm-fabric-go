...
func Fetch(source string, callback Callback, logger zerolog.Logger) error {
	...
	var walker = func(absolute string, info os.FileInfo, err error) error {
		...
		if err != nil {
			return errors.Wrapf(err, "error walking the template directory %s", destination) 
		}

		if relative, err = filepath.Rel(destination, absolute); err != nil {
			return errors.Wrapf(err, "error determining the relative path from %s to %s", destination, absolute) 
		}
	...
	if err = filepath.Walk(destination, walker); err != nil {
		return errors.Wrapf(err, "error invoking callback for fetcher") 
	}
